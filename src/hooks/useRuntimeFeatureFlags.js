/**
 * useRuntimeFeatureFlags - Hook para feature flags dinÃ¡micos desde Firestore
 */
 ...
 * @param {string} flagName - Nombre especÃ­fico del flag (opcional)
...
export function useRuntimeFeatureFlags(flagName) {
  ...
  // âœ… RESET cuando cambia el orgId
  useEffect(() => {
  ...
  useEffect(() => {
    // âœ… SOLUCIÃ“N ROBUSTA: PatrÃ³n de suscripciÃ³n reactiva
    ...
    // Estado 2: useMultiTenant terminÃ³, pero no hay orgId
    if (!currentOrgId) {
  ...
  // Si se pidiÃ³ un flag especÃ­fico, retornar solo ese
  if (flagName) {
    return {
