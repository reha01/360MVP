...
import { isPublicHost } from '../utils/env';
...
// Conectar emuladores solo si NO estamos en un host público
const useEmulators = import.meta.env.VITE_USE_EMULATORS === 'true';

if (isPublicHost()) {
  console.log('[360MVP] Emulators disabled on public host');
} else if (useEmulators && (location.hostname === "127.0.0.1" || location.hostname === "localhost")) {
  try {
    // Auth Emulator
