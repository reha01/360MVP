...
import { testSafeSetInterval, shouldDisableAnalytics } from '../utils/testingUtils.js';
...
class ScopedAnalyticsService {
  ...
  /**
   * Obtiene mÃ©tricas agregadas de la organizaciÃ³n
   */
  ...
  subscribeToMetrics(userId, callback, timeRange = '30d') {
    ...
    if (shouldDisableAnalytics()) {
      console.log('[ScopedAnalyticsService] ðŸ§ª subscribeToMetrics disabled - testing environment');
    ...
    // âœ… USAR testSafeSetInterval en lugar de setInterval directo
    const pollInterval = testSafeSetInterval(async () => {
      try {
    ...
    // âœ… Solo agregar unsubscriber si pollInterval no es null
    if (pollInterval !== null) {
      this.unsubscribers.set(unsubscribeKey, () => clearInterval(pollInterval));
    }
    
    // Initial load (solo si analytics estÃ¡ habilitado)
    if (!shouldDisableAnalytics()) {
