...
class ScopedAnalyticsService {
  ...
  subscribeToMetrics(userId, callback, timeRange = '30d') {
    ...
    // ✅ KILL SWITCH: Deshabilitar en entorno de testing
    if (shouldDisableAnalytics()) {
      console.log('[ScopedAnalyticsService] 🧪 subscribeToMetrics disabled - testing environment');
    ...
    // ✅ USAR testSafeSetInterval en lugar de setInterval directo
    ...
    // ✅ Solo agregar unsubscriber si pollInterval no es null
    if (pollInterval !== null) {
      this.unsubscribers.set(unsubscribeKey, () => clearInterval(pollInterval));
    }
    
    // Initial load (solo si analytics está habilitado)
    if (!shouldDisableAnalytics()) {
