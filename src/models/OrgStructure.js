...
export const createExtendedUserModel = (data) => {
  ...
  return {
    ...
    // Relaciones jerárquicas (UPDATED for hierarchy management)
    managerIds: data.managerIds || [], // Array para soportar jefaturas múltiples (matriz)
    managerId: data.managerId || null, // Legacy - se mantiene por compatibilidad
    managerType: data.managerType || MANAGER_TYPES.FUNCTIONAL,
...
export const validateExtendedUser = (user) => {
  ...
  // Validación de managerIds (nuevo)
  if (user.managerIds && !Array.isArray(user.managerIds)) {
    errors.push('Manager IDs debe ser un array');
  }

  // Validación de status (nuevo)
  const validStatuses = ['active', 'inactive', 'archived'];
  if (user.status && !validStatuses.includes(user.status)) {
    errors.push(`Status inválido: ${user.status}. Debe ser: ${validStatuses.join(', ')}`);
  }
